...
When("informar o nome de um filme não cadastrado", () => {
  inicialPage.typePesquisa("AriErikoLariManoscreçççu");
});
...
Then("será possível visualizar o filme pesquisado", () => {
  cy.contains(inicialPage.movieTitle, filme.title).should("be.visible");
});
...
Then(
  "será possível visualizar lista de filmes que contem o que foi pesquisado",
  () => {
    cy.get(inicialPage.movieTitle)
      .contains(filme.title.slice(0, 3))
      .and("be.visible");
  }
);
