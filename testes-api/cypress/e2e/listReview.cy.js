...
describe('Listagem de avaliações de filmes', function () {
    ...
    beforeEach(() => {
        cy.adminCreatesAMovie(faker.animal.cat(), faker.person.firstName(), faker.lorem.sentence(), 180, 2024).then(function (response) {
    ...
    it('Deve ser possível um usuário comum listar as próprias avaliações', function () {
        ...
        cy.createAndLoginUser(faker.person.firstName() + 'ão', faker.internet.exampleEmail(), '123456').then(function (response) {
            ...
            cy.request({
                ...
                expect(response.body[0].reviewText).to.equal("Teste review usuário inativado / promovido")
    ...
    it('Não deve ser possível um usuário não autenticado a listar avaliações', function () {
        let token;
        cy.createAndLoginUser(faker.person.firstName() + 'ão', faker.internet.exampleEmail(), '123456').then(function (response) {
            token = response.token
            cy.postReview(movieId, token)
