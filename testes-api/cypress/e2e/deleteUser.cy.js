...
describe('Deletar conta', ()=>{
    ...
    it('Administrador deve ser capaz de excluir a conta de qualquer usuário', ()=>{
        ...
        cy.createUser('Joelma', faker.internet.exampleEmail(), 'dancarcurtir').then((response)=>{
    ...
    it('Usuário comum não deve conseguir excluir conta de outro usuário', ()=>{
        cy.createAndLoginUser('Erika Hilton', faker.internet.exampleEmail(), faker.internet.password({length: 10})).then((response)=>{
             token = response.token
            cy.deleteUser(userId, "Bearer" + token, false).then((response)=>{
                expect(response.body).to.deep.equal({
                    "message": "Access denied.",
    ...
    it('Usuário critico não deve conseguir excluir conta de outro usuário', ()=>{
        cy.createAndLoginCritic('Urias', faker.internet.exampleEmail(), faker.internet.password({length: 10})).then((response)=>{
             token = response.token
            cy.deleteUser(userId, "Bearer" + token, false).then((response)=>{
                expect(response.body).to.deep.equal({
                    "message": "Access denied.",
