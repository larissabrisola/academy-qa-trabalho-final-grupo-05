...
describe("Cadastro de usuário", () => {
  ...
  it("Deve ser possivel cadastrar com nome contendo 100 caracteres", () => {
    cy.createUser(
      "assistamJaneTheVirginMelhoreSerieassistamJaneTheVirginMelhoreSerieassistamJaneTheVirginMelhoreSerieA",
      faker.internet.exampleEmail(),
      faker.internet.password({ length: 10 }),
      true
    ).then((response) => {
  ...
  it("Deve ser possivel cadastrar com email contendo 60 caracteres", () => {
    cy.createUser(
      "Jane",
      faker.string.alpha(50) + "@teste.com",
      faker.internet.password({ length: 10 })
    ).then((response) => {
      expect(response.status).to.equal(201);
      expect(response.body).to.have.property("id");
  ...
  it("Não deve ser possivel cadastrar com um email já utilizado", () => {
    ...
    cy.createUser(randomName, randomEmail, "aboboradoi", false).then(
      (response) => {
  ...
  it("Não deve ser possivel cadastrar com email contendo 61 ou mais caracteres", () => {
    cy.createUser(
      "Michael",
      faker.string.alpha(50) + "@testes.com",
      "linuxtips",
      false
    ).then((response) => {
      expect(response.body).to.deep.equal({
  ...
  it("Não deve ser possivel cadastrar com senha contendo 13 ou mais caracteres", () => {
    cy.createUser(
      "Michael",
      faker.internet.exampleEmail(),
      faker.internet.password({ length: 13 }),
      false
    ).then((response) => {
      expect(response.body).to.deep.equal({
  ...
  it("Não deve ser possivel cadastrar se o campo senha estiver vazio", () => {
    cy.createUser("Cirilo", faker.internet.exampleEmail(), "", false).then(
      (response) => {
        expect(response.body).to.deep.equal({
          message: [
