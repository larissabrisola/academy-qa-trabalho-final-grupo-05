...
describe('Criar review', function () {
    let userName;
    let userEmail;
    let id;
    let token;
    ...
    beforeEach(() => {
        ...
        cy.createAndLogAdmin(userName + 'ão', userEmail, '123456').then(function (response) {
    ...
    it('Deve ser possível um usuário administrador criar review de um filme', function () {
        cy.createAndLogAdmin('Joelma', faker.internet.exampleEmail(), '123456').then(function (response) {
            token = response.token
            cy.get('@filmeCriado').then(function (filme) {
                cy.request({
                    method: "POST",
    ...
    it('Deve ser possível escrever review informando 500 caracteres', function () {
        cy.createAndLoginUser('Janaina', faker.internet.exampleEmail(), '123456').then(function (response) {
            token = response.token
            cy.promoteCritic(token)
            cy.get('@filmeCriado').then(function (filme) {
    ...
    it('Não deve ser possível escrever review informando 501 ou mais caracteres', function () {
        cy.createAndLoginUser('Janaina', faker.internet.exampleEmail(), '123456').then(function (response) {
            token = response.token
            cy.promoteCritic(token)
            cy.get('@filmeCriado').then(function (filme) {
...
it('Não deve ser possível atribuir nota 0 ao filme', function () {
...
it('Não deve ser possível um filme ter uma nota superior a 5', function () {
    cy.createAndLoginUser(faker.animal.horse(), faker.internet.exampleEmail(), '123456').then(function (response) {
        token = response.token
        cy.promoteAdmin(token)
        cy.get('@filmeCriado').then(function (filme) {
...
it('Não deve ser possível criar review sem informar o ID', function () {
    cy.createAndLoginUser(faker.animal.horse(), faker.internet.exampleEmail(), '123456').then(function (response) {
        token = response.token
...
it('Não deve ser possível criar review sem informar a nota do filme', function () {
    cy.createAndLoginUser(faker.animal.horse(), faker.internet.exampleEmail(), '123456').then(function (response) {
        token = response.token
        cy.get('@filmeCriado').then(function (filme) {
            cy.request({
    ...
    it('Não deve ser possível atribuir nota usando numeros decimais', function () {
        cy.createAndLoginUser(faker.animal.horse(), faker.internet.exampleEmail(), '123456').then(function (response) {
            token = response.token
            cy.promoteAdmin(token)
            cy.get('@filmeCriado').then(function (filme) {
